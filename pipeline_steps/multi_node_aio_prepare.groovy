    ...
    stage: {
      ...
      dir("openstack-ansible-ops/${env.MULTI_NODE_AIO_DIR}") {
        timeout(time: 45, unit: "MINUTES") {
          sh """
          sed -i "s,<memory unit='GiB'>1,<memory unit='GiB'>4," templates/vmnode-config/deploy.openstackci.local.xml
          sed -i "s,<currentMemory unit='GiB'>1,<currentMemory unit='GiB'>4," templates/vmnode-config/deploy.openstackci.local.xml
